/**
 * CusCtmAdrCrr Entity Listener 클래스
 *
...
@Slf4j
public class CusCtmAdrCrrListener {
    ...
    //----------------------------------------------------
    @PostLoad
    public void onPostLoad(CusCtmAdrCrr postData) {
        log.info("onPostLoad : Select후 호출({})" , postData.toString() );
    }

    @PrePersist
    public void onPrePersist(CusCtmAdrCrr preData) {
        log.info("onPrePersist : Insert전 호출({})" , preData.toString() );
    }

    @PostPersist
    public void onPostPersist(CusCtmAdrCrr postData) {
        log.info("onPrePersist : Insert후 호출({})" , postData.toString() );
    }

    @PreUpdate
    public void onPreUpdate(CusCtmAdrCrr preData) {
        log.info("onPreUpdate : Update전 호출({})" , preData.toString() );
    }

    @PostUpdate
    public void onPostUpdate(CusCtmAdrCrr postData) {
        log.info("onPostUpdate : Update후 호출({})" , postData.toString() );
    }

    @PreRemove
    public void onPreRemove(CusCtmAdrCrr preData) {
        log.info("onPreRemove  : Delete전 호출({})" , preData.toString() );
    }

    @PostRemove
    public void onPostRemove(CusCtmAdrCrr postData) {
