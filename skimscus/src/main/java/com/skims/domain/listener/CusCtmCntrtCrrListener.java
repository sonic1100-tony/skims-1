/**
 * CusCtmCntrtCrr Entity Listener 클래스
 *
...
@Slf4j
public class CusCtmCntrtCrrListener {
    ...
    //----------------------------------------------------
    @PostLoad
    public void onPostLoad(CusCtmCntrtCrr postData) {
        log.info("onPostLoad : Select후 호출({})" , postData.toString() );
    }

    @PrePersist
    public void onPrePersist(CusCtmCntrtCrr preData) {
        log.info("onPrePersist : Insert전 호출({})" , preData.toString() );
    }

    @PostPersist
    public void onPostPersist(CusCtmCntrtCrr postData) {
        log.info("onPrePersist : Insert후 호출({})" , postData.toString() );
    }

    @PreUpdate
    public void onPreUpdate(CusCtmCntrtCrr preData) {
        log.info("onPreUpdate : Update전 호출({})" , preData.toString() );
    }

    @PostUpdate
    public void onPostUpdate(CusCtmCntrtCrr postData) {
        log.info("onPostUpdate : Update후 호출({})" , postData.toString() );
    }

    @PreRemove
    public void onPreRemove(CusCtmCntrtCrr preData) {
        log.info("onPreRemove  : Delete전 호출({})" , preData.toString() );
    }

    @PostRemove
    public void onPostRemove(CusCtmCntrtCrr postData) {
