/**
 * CusPstpoRcpl Entity Listener 클래스
 *
...
@Slf4j
public class CusPstpoRcplListener {
    ...
    //----------------------------------------------------
    @PostLoad
    public void onPostLoad(CusPstpoRcpl postData) {
        log.info("onPostLoad : Select후 호출({})" , postData.toString() );
    }

    @PrePersist
    public void onPrePersist(CusPstpoRcpl preData) {
        log.info("onPrePersist : Insert전 호출({})" , preData.toString() );
    }

    @PostPersist
    public void onPostPersist(CusPstpoRcpl postData) {
        log.info("onPrePersist : Insert후 호출({})" , postData.toString() );
    }

    @PreUpdate
    public void onPreUpdate(CusPstpoRcpl preData) {
        log.info("onPreUpdate : Update전 호출({})" , preData.toString() );
    }

    @PostUpdate
    public void onPostUpdate(CusPstpoRcpl postData) {
        log.info("onPostUpdate : Update후 호출({})" , postData.toString() );
    }

    @PreRemove
    public void onPreRemove(CusPstpoRcpl preData) {
        log.info("onPreRemove  : Delete전 호출({})" , preData.toString() );
    }

    @PostRemove
    public void onPostRemove(CusPstpoRcpl postData) {
