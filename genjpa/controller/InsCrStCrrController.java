...
public class InsCrStCrrController {
    ...
    public ResponseEntity<Page<InsCrStCrr>> getAll(@PageableDefault Pageable pageable) {
        try {
			log.info("findAll");
    ...
    ResponseEntity<InsCrStCrr> putData(@RequestBody InsCrStCrr newData, @PathVariable("aid") Long aid) {
        return repository.findById(aid) //
                .map(oldData -> {
                    ...
                    oldData.setCrStadt(newData.getCrStadt());
                    oldData.setPartCvrStcd(newData.getPartCvrStcd());
                ...
                .map(oldData -> {
                    newMap.forEach((strKey, strValue) -> {
                        ...
                        if (strKey.equals("crDtStcd"))
                            oldData.setCrDtStcd(newData.getCrDtStcd());
                        if (strKey.equals("crStadt"))
                            oldData.setCrStadt(newData.getCrStadt());
                        if (strKey.equals("partCvrStcd"))
                            oldData.setPartCvrStcd(newData.getPartCvrStcd());
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Delete the InsCrStCrr", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = InsCrStCrr.class)) }),
            @ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content) })
