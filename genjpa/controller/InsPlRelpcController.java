/**
 * InsPlRelpc Entity Controller 클래스
 *
...
@RestController
@Tag(name = "InsPlRelpc 설계관계자 Controller")
@Slf4j
public class InsPlRelpcController {

    @Autowired
    InsPlRelpcRepository repository;


    @Autowired
    ObjectMapper mapper;

    // @Operation(summary = "설계관계자 전체조회" , summary = "설계관계자 조회" )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Found the insplrelpcs", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = InsPlRelpc.class)) }),
            @ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content) })
    @GetMapping("/insplrelpcs")
    public ResponseEntity<Page<InsPlRelpc>> getAll(@PageableDefault Pageable pageable) {
        try {
			log.info("findAll");
    ...
    // @Operation(summary = "설계관계자 Key조회" , description = "설계관계자 Primary Key로 조회" )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Found the InsPlRelpc", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = InsPlRelpc.class)) }),
            @ApiResponse(responseCode = "404", description = "InsPlRelpc not found", content = @Content) })
    @GetMapping("/insplrelpcs/{aid}")
    public ResponseEntity<InsPlRelpc> getById(@PathVariable("aid") Long aid) {
        Optional<InsPlRelpc> data = repository.findById(aid);
    ...
    // @Operation(summary = "설계관계자 등록" , description = "설계관계자 신규 데이터 등록" )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Create the InsPlRelpc", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = InsPlRelpc.class)) }),
            @ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content) })
    @PostMapping("/insplrelpcs")
    ResponseEntity<InsPlRelpc> postData(@RequestBody InsPlRelpc newData) {
        try {
    ...
    // @Operation(summary = "설계관계자 수정" , description = "설계관계자 데이터 수정" )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Update the InsPlRelpc", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = InsPlRelpc.class)) }),
            @ApiResponse(responseCode = "404", description = "InsPlRelpc not found", content = @Content) })
    @PutMapping("/insplrelpcs/{aid}")
    ResponseEntity<InsPlRelpc> putData(@RequestBody InsPlRelpc newData, @PathVariable("aid") Long aid) {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Update the InsPlRelpc", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = InsPlRelpc.class)) }),
            @ApiResponse(responseCode = "404", description = "InsPlRelpc not found", content = @Content) })
	@PatchMapping("/insplrelpcs/{aid}")
    ResponseEntity<InsPlRelpc> patchData(@RequestBody Map<String, Object> newMap, @PathVariable("aid") Long aid) {
        InsPlRelpc newData = mapper.convertValue(newMap, InsPlRelpc.class);
        return repository.findById(aid) //
                .map(oldData -> {
    ...
    // @Operation(summary = "설계관계자 삭제" , description = "설계관계자 Primary Key로 삭제" )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Delete the InsPlRelpc", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = InsPlRelpc.class)) }),
            @ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content) })
    @DeleteMapping("/insplrelpcs/{aid}")
