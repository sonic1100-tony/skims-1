/**
 * InsInsPl Entity Controller 클래스
 *
...
@RestController
@Tag(name = "InsInsPl 보험설계 Controller")
@Slf4j
public class InsInsPlController {

    @Autowired
    InsInsPlRepository repository;


    @Autowired
    ObjectMapper mapper;

    // @Operation(summary = "보험설계 전체조회" , summary = "보험설계 조회" )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Found the insinspls", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = InsInsPl.class)) }),
            @ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content) })
    @GetMapping("/insinspls")
    public ResponseEntity<Page<InsInsPl>> getAll(@PageableDefault Pageable pageable) {
        try {
			log.info("findAll");
    ...
    // @Operation(summary = "보험설계 Key조회" , description = "보험설계 Primary Key로 조회" )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Found the InsInsPl", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = InsInsPl.class)) }),
            @ApiResponse(responseCode = "404", description = "InsInsPl not found", content = @Content) })
    @GetMapping("/insinspls/{aid}")
    public ResponseEntity<InsInsPl> getById(@PathVariable("aid") Long aid) {
        Optional<InsInsPl> data = repository.findById(aid);
    ...
    // @Operation(summary = "보험설계 등록" , description = "보험설계 신규 데이터 등록" )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Create the InsInsPl", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = InsInsPl.class)) }),
            @ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content) })
    @PostMapping("/insinspls")
    ResponseEntity<InsInsPl> postData(@RequestBody InsInsPl newData) {
        try {
    ...
    // @Operation(summary = "보험설계 수정" , description = "보험설계 데이터 수정" )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Update the InsInsPl", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = InsInsPl.class)) }),
            @ApiResponse(responseCode = "404", description = "InsInsPl not found", content = @Content) })
    @PutMapping("/insinspls/{aid}")
    ResponseEntity<InsInsPl> putData(@RequestBody InsInsPl newData, @PathVariable("aid") Long aid) {
        return repository.findById(aid) //
                .map(oldData -> {
                    oldData.setPlno(newData.getPlno());
                    oldData.setCgafChSeqno(newData.getCgafChSeqno());
                    oldData.setPldt(newData.getPldt());
                    oldData.setValdPlYn(newData.getValdPlYn());
                    oldData.setPlFlgcd(newData.getPlFlgcd());
                    oldData.setCmMtdcd(newData.getCmMtdcd());
                    oldData.setPlStcd(newData.getPlStcd());
                    oldData.setPlStChdt(newData.getPlStChdt());
                    oldData.setFstiDmMtdcd(newData.getFstiDmMtdcd());
                    oldData.setIsPlKndcd(newData.getIsPlKndcd());
                    oldData.setXpDmgrt(newData.getXpDmgrt());
                    oldData.setPlCmprGpno(newData.getPlCmprGpno());
                    oldData.setRecPlno(newData.getRecPlno());
                    oldData.setRecCgaocSeqno(newData.getRecCgaocSeqno());
                    oldData.setRecCorcd(newData.getRecCorcd());
                    oldData.setPlValdPerdt(newData.getPlValdPerdt());
                    oldData.setSpccfPlno(newData.getSpccfPlno());
                    oldData.setSpccfCgafChSeqno(newData.getSpccfCgafChSeqno());
                    oldData.setNdPlTpcd(newData.getNdPlTpcd());
                    oldData.setUdrtkGuVltYn(newData.getUdrtkGuVltYn());
                    oldData.setPlChPsbdt(newData.getPlChPsbdt());
                    oldData.setDgnBjYn(newData.getDgnBjYn());
                    oldData.setRqRcpno(newData.getRqRcpno());
                    oldData.setPlyno(newData.getPlyno());
                    oldData.setCnrdt(newData.getCnrdt());
                    oldData.setPprPlno(newData.getPprPlno());
                    oldData.setPprCgaocSeqno(newData.getPprCgaocSeqno());
                    oldData.setSlrTfStfno(newData.getSlrTfStfno());
                    oldData.setCrtSlrtfStfRelcd(newData.getCrtSlrtfStfRelcd());
                    oldData.setRviPlno(newData.getRviPlno());
                    oldData.setIsPlTycd(newData.getIsPlTycd());
                    oldData.setQaUdBjyn(newData.getQaUdBjyn());
                    oldData.setRvibaNdsDlFlgcd(newData.getRvibaNdsDlFlgcd());
                    ...
                    oldData.setPlCt(newData.getPlCt());
                    oldData.setDcApPlCt(newData.getDcApPlCt());
                    oldData.setGrRvSbStcd(newData.getGrRvSbStcd());
                    ...
                    oldData.setMwWlamt(newData.getMwWlamt());
                    oldData.setFrMwwdrRndYyct(newData.getFrMwwdrRndYyct());
                    oldData.setMwWdraSt(newData.getMwWdraSt());
                    ...
                    oldData.setPlStDtcd(newData.getPlStDtcd());
                    oldData.setUdAsTpcd(newData.getUdAsTpcd());
                    oldData.setBefoCgaocSeqno(newData.getBefoCgaocSeqno());
                    oldData.setBefoPlno(newData.getBefoPlno());
                    oldData.setGuPlyno(newData.getGuPlyno());
                    oldData.setPrnotTrnmMtdcd(newData.getPrnotTrnmMtdcd());
                    oldData.setPlRcpTpcd(newData.getPlRcpTpcd());
                    oldData.setPlRcpno(newData.getPlRcpno());
                    oldData.setSmsSndYn(newData.getSmsSndYn());
                    oldData.setLtranPlno(newData.getLtranPlno());
                    oldData.setLtranCgafChSeqno(newData.getLtranCgafChSeqno());
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Update the InsInsPl", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = InsInsPl.class)) }),
            @ApiResponse(responseCode = "404", description = "InsInsPl not found", content = @Content) })
	@PatchMapping("/insinspls/{aid}")
    ResponseEntity<InsInsPl> patchData(@RequestBody Map<String, Object> newMap, @PathVariable("aid") Long aid) {
        InsInsPl newData = mapper.convertValue(newMap, InsInsPl.class);
        return repository.findById(aid) //
                .map(oldData -> {
                    newMap.forEach((strKey, strValue) -> {
                        ...
                        if (strKey.equals("cgafChSeqno"))
                            oldData.setCgafChSeqno(newData.getCgafChSeqno());
                        if (strKey.equals("pldt"))
                            oldData.setPldt(newData.getPldt());
                        if (strKey.equals("valdPlYn"))
                            oldData.setValdPlYn(newData.getValdPlYn());
                        if (strKey.equals("plFlgcd"))
                            oldData.setPlFlgcd(newData.getPlFlgcd());
                        if (strKey.equals("cmMtdcd"))
                            oldData.setCmMtdcd(newData.getCmMtdcd());
                        if (strKey.equals("plStcd"))
                            oldData.setPlStcd(newData.getPlStcd());
                        if (strKey.equals("plStChdt"))
                            oldData.setPlStChdt(newData.getPlStChdt());
                        if (strKey.equals("fstiDmMtdcd"))
                            oldData.setFstiDmMtdcd(newData.getFstiDmMtdcd());
                        if (strKey.equals("isPlKndcd"))
                            oldData.setIsPlKndcd(newData.getIsPlKndcd());
                        if (strKey.equals("xpDmgrt"))
                            oldData.setXpDmgrt(newData.getXpDmgrt());
                        if (strKey.equals("plCmprGpno"))
                            oldData.setPlCmprGpno(newData.getPlCmprGpno());
                        if (strKey.equals("recPlno"))
                            oldData.setRecPlno(newData.getRecPlno());
                        if (strKey.equals("recCgaocSeqno"))
                            oldData.setRecCgaocSeqno(newData.getRecCgaocSeqno());
                        if (strKey.equals("recCorcd"))
                            oldData.setRecCorcd(newData.getRecCorcd());
                        if (strKey.equals("plValdPerdt"))
                            oldData.setPlValdPerdt(newData.getPlValdPerdt());
                        if (strKey.equals("spccfPlno"))
                            oldData.setSpccfPlno(newData.getSpccfPlno());
                        if (strKey.equals("spccfCgafChSeqno"))
                            oldData.setSpccfCgafChSeqno(newData.getSpccfCgafChSeqno());
                        if (strKey.equals("ndPlTpcd"))
                            oldData.setNdPlTpcd(newData.getNdPlTpcd());
                        if (strKey.equals("udrtkGuVltYn"))
                            oldData.setUdrtkGuVltYn(newData.getUdrtkGuVltYn());
                        if (strKey.equals("plChPsbdt"))
                            oldData.setPlChPsbdt(newData.getPlChPsbdt());
                        if (strKey.equals("dgnBjYn"))
                            oldData.setDgnBjYn(newData.getDgnBjYn());
                        if (strKey.equals("rqRcpno"))
                            oldData.setRqRcpno(newData.getRqRcpno());
                        if (strKey.equals("plyno"))
                            oldData.setPlyno(newData.getPlyno());
                        if (strKey.equals("cnrdt"))
                            oldData.setCnrdt(newData.getCnrdt());
                        if (strKey.equals("pprPlno"))
                            oldData.setPprPlno(newData.getPprPlno());
                        if (strKey.equals("pprCgaocSeqno"))
                            oldData.setPprCgaocSeqno(newData.getPprCgaocSeqno());
                        if (strKey.equals("slrTfStfno"))
                            oldData.setSlrTfStfno(newData.getSlrTfStfno());
                        if (strKey.equals("crtSlrtfStfRelcd"))
                            oldData.setCrtSlrtfStfRelcd(newData.getCrtSlrtfStfRelcd());
                        if (strKey.equals("rviPlno"))
                            oldData.setRviPlno(newData.getRviPlno());
                        if (strKey.equals("isPlTycd"))
                            oldData.setIsPlTycd(newData.getIsPlTycd());
                        if (strKey.equals("qaUdBjyn"))
                            oldData.setQaUdBjyn(newData.getQaUdBjyn());
                        if (strKey.equals("rvibaNdsDlFlgcd"))
                            oldData.setRvibaNdsDlFlgcd(newData.getRvibaNdsDlFlgcd());
                        ...
                        if (strKey.equals("ltrmNdsDlFlgcd"))
                            oldData.setLtrmNdsDlFlgcd(newData.getLtrmNdsDlFlgcd());
                        if (strKey.equals("plCt"))
                            oldData.setPlCt(newData.getPlCt());
                        if (strKey.equals("dcApPlCt"))
                            oldData.setDcApPlCt(newData.getDcApPlCt());
                        if (strKey.equals("grRvSbStcd"))
                            oldData.setGrRvSbStcd(newData.getGrRvSbStcd());
                        ...
                        if (strKey.equals("mwPyMtdcd"))
                            oldData.setMwPyMtdcd(newData.getMwPyMtdcd());
                        if (strKey.equals("mwWlamt"))
                            oldData.setMwWlamt(newData.getMwWlamt());
                        if (strKey.equals("frMwwdrRndYyct"))
                            oldData.setFrMwwdrRndYyct(newData.getFrMwwdrRndYyct());
                        if (strKey.equals("mwWdraSt"))
                            oldData.setMwWdraSt(newData.getMwWdraSt());
                        ...
                        if (strKey.equals("nwcrScanCpltDthms"))
                            oldData.setNwcrScanCpltDthms(newData.getNwcrScanCpltDthms());
                        if (strKey.equals("plStDtcd"))
                            oldData.setPlStDtcd(newData.getPlStDtcd());
                        if (strKey.equals("udAsTpcd"))
                            oldData.setUdAsTpcd(newData.getUdAsTpcd());
                        if (strKey.equals("befoCgaocSeqno"))
                            oldData.setBefoCgaocSeqno(newData.getBefoCgaocSeqno());
                        if (strKey.equals("befoPlno"))
                            oldData.setBefoPlno(newData.getBefoPlno());
                        if (strKey.equals("guPlyno"))
                            oldData.setGuPlyno(newData.getGuPlyno());
                        if (strKey.equals("prnotTrnmMtdcd"))
                            oldData.setPrnotTrnmMtdcd(newData.getPrnotTrnmMtdcd());
                        if (strKey.equals("plRcpTpcd"))
                            oldData.setPlRcpTpcd(newData.getPlRcpTpcd());
                        if (strKey.equals("plRcpno"))
                            oldData.setPlRcpno(newData.getPlRcpno());
                        if (strKey.equals("smsSndYn"))
                            oldData.setSmsSndYn(newData.getSmsSndYn());
                        if (strKey.equals("ltranPlno"))
                            oldData.setLtranPlno(newData.getLtranPlno());
                        if (strKey.equals("ltranCgafChSeqno"))
                            oldData.setLtranCgafChSeqno(newData.getLtranCgafChSeqno());
