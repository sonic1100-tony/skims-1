/**
 * FinLdgAccCrr Entity Listener 클래스
 *
...
@Slf4j
public class FinLdgAccCrrListener {
    ...
    //----------------------------------------------------
    @PostLoad
    public void onPostLoad(FinLdgAccCrr postData) {
        log.info("onPostLoad : Select후 호출({})" , postData.toString() );
    }

    @PrePersist
    public void onPrePersist(FinLdgAccCrr preData) {
        log.info("onPrePersist : Insert전 호출({})" , preData.toString() );
    }

    @PostPersist
    public void onPostPersist(FinLdgAccCrr postData) {
        log.info("onPrePersist : Insert후 호출({})" , postData.toString() );
    }

    @PreUpdate
    public void onPreUpdate(FinLdgAccCrr preData) {
        log.info("onPreUpdate : Update전 호출({})" , preData.toString() );
    }

    @PostUpdate
    public void onPostUpdate(FinLdgAccCrr postData) {
        log.info("onPostUpdate : Update후 호출({})" , postData.toString() );
    }

    @PreRemove
    public void onPreRemove(FinLdgAccCrr preData) {
        log.info("onPreRemove  : Delete전 호출({})" , preData.toString() );
    }

    @PostRemove
    public void onPostRemove(FinLdgAccCrr postData) {
