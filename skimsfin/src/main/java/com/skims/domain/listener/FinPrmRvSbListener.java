/**
 * FinPrmRvSb Entity Listener 클래스
 *
...
@Slf4j
public class FinPrmRvSbListener {
    ...
    //----------------------------------------------------
    @PostLoad
    public void onPostLoad(FinPrmRvSb postData) {
        log.info("onPostLoad : Select후 호출({})" , postData.toString() );
    }

    @PrePersist
    public void onPrePersist(FinPrmRvSb preData) {
        log.info("onPrePersist : Insert전 호출({})" , preData.toString() );
    }

    @PostPersist
    public void onPostPersist(FinPrmRvSb postData) {
        log.info("onPrePersist : Insert후 호출({})" , postData.toString() );
    }

    @PreUpdate
    public void onPreUpdate(FinPrmRvSb preData) {
        log.info("onPreUpdate : Update전 호출({})" , preData.toString() );
    }

    @PostUpdate
    public void onPostUpdate(FinPrmRvSb postData) {
        log.info("onPostUpdate : Update후 호출({})" , postData.toString() );
    }

    @PreRemove
    public void onPreRemove(FinPrmRvSb preData) {
        log.info("onPreRemove  : Delete전 호출({})" , preData.toString() );
    }

    @PostRemove
    public void onPostRemove(FinPrmRvSb postData) {
