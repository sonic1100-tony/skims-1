/**
 * IgdLtrmGdRtInfo Entity Listener 클래스
 *
...
@Slf4j
public class IgdLtrmGdRtInfoListener {
    ...
    //----------------------------------------------------
    @PostLoad
    public void onPostLoad(IgdLtrmGdRtInfo postData) {
        log.info("onPostLoad : Select후 호출({})" , postData.toString() );
    }

    @PrePersist
    public void onPrePersist(IgdLtrmGdRtInfo preData) {
        log.info("onPrePersist : Insert전 호출({})" , preData.toString() );
    }

    @PostPersist
    public void onPostPersist(IgdLtrmGdRtInfo postData) {
        log.info("onPrePersist : Insert후 호출({})" , postData.toString() );
    }

    @PreUpdate
    public void onPreUpdate(IgdLtrmGdRtInfo preData) {
        log.info("onPreUpdate : Update전 호출({})" , preData.toString() );
    }

    @PostUpdate
    public void onPostUpdate(IgdLtrmGdRtInfo postData) {
        log.info("onPostUpdate : Update후 호출({})" , postData.toString() );
    }

    @PreRemove
    public void onPreRemove(IgdLtrmGdRtInfo preData) {
        log.info("onPreRemove  : Delete전 호출({})" , preData.toString() );
    }

    @PostRemove
    public void onPostRemove(IgdLtrmGdRtInfo postData) {
