/**
 * IgdLtrmBzcs Entity Listener 클래스
 *
...
@Slf4j
public class IgdLtrmBzcsListener {
    ...
    //----------------------------------------------------
    @PostLoad
    public void onPostLoad(IgdLtrmBzcs postData) {
        log.info("onPostLoad : Select후 호출({})" , postData.toString() );
    }

    @PrePersist
    public void onPrePersist(IgdLtrmBzcs preData) {
        log.info("onPrePersist : Insert전 호출({})" , preData.toString() );
    }

    @PostPersist
    public void onPostPersist(IgdLtrmBzcs postData) {
        log.info("onPrePersist : Insert후 호출({})" , postData.toString() );
    }

    @PreUpdate
    public void onPreUpdate(IgdLtrmBzcs preData) {
        log.info("onPreUpdate : Update전 호출({})" , preData.toString() );
    }

    @PostUpdate
    public void onPostUpdate(IgdLtrmBzcs postData) {
        log.info("onPostUpdate : Update후 호출({})" , postData.toString() );
    }

    @PreRemove
    public void onPreRemove(IgdLtrmBzcs preData) {
        log.info("onPreRemove  : Delete전 호출({})" , preData.toString() );
    }

    @PostRemove
    public void onPostRemove(IgdLtrmBzcs postData) {
